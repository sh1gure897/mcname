...
class Checkpoint:
    ...
    def _load(self):
        # A missing or broken checkpoint just means "start over" - don't blow up
        # a fresh run because of a bad file.
...
class NameChecker:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                async with session.get(url, timeout=timeout) as response:
                    ...
                    # 5xx and friends - treat as a blip, retry on the same
                    # schedule instead of throwing the name away.
    ...
    async def _record_available(self, candidate: NameCandidate):
    ...
    async def _process_candidate(self, session, notifier, candidate, progress):
        async with self._semaphore:
            if self._stop_event.is_set():
                return

            # Small delay + jitter spreads the requests out a bit so we don't
    ...
    async def run(self):
        # Drop anything already in the checkpoint before we schedule any tasks.
        all_candidates = [
            candidate
        ...
        log.info(
            "Scanning %d candidate name(s) for length(s) %s.", total, self._lengths
        )
        ...
        async with aiohttp.ClientSession(connector=connector) as session:
            ...
            with tqdm(total=total, unit="name", desc="Checking") as progress:
                ...
                try:
                    ...
                    # Save no matter what - Ctrl-C, exception, normal finish.
...
def _install_signal_handlers(checker: "NameChecker"):
    # Windows' asyncio loop has no add_signal_handler; there Ctrl-C still
    # raises KeyboardInterrupt and the finally block saves anyway, so this is
    # just a nicety where it works.
